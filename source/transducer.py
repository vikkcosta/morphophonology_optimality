...
class Transducer:
    ...
    def get_arcs_by_origin_state(self, origin_state):
        ...
        if origin_state in self.arcs_by_state_dict:
            for state_arcs in self.arcs_by_state_dict[origin_state].values():
                arcs.extend(state_arcs)
        return arcs

    def get_arcs_by_terminal_state(self, terminal_state):
        arcs = list()
        for state_arcs in self.arcs_by_state_dict.values():
    ...
    def _arcs_dot_representation(self):
        ...
        for arc in arcs:
            if arc.origin_state == arc.terminal_state:
                ...
                print("\"{0}\" -> \"{1}\" [label=\"{2} : {3} {4}\\n\"];".format(
                    arc.origin_state, arc.terminal_state, arc.input.get_symbol(), get_output_str(arc.output), arc.cost_vector), file=string_io, end="\n")
        ...
        for state in loop_arcs_by_state:
            ...
            print("\"{0}\" -> \"{1}\" [label=\"{2}\"];".format(
                     state, state, combined_label), file=string_io, end="\n")
        ...
        for states_tuple in multiple_arcs_state_tuple:
            ...
            print("\"{0}\" -> \"{1}\" [label=\"{2}\"];".format(
                     state1, state2, combined_label), file=string_io, end="\n")

        return string_io.getvalue()

    def dot_representation(self):
        string_io = StringIO()
        print("digraph acceptor {", file=string_io, end="\n")
        print("rankdir=LR", file=string_io, end="\n")
        print("size=\"11,5\"", file=string_io, end="\n")
        print("node [shape = ellipse];", file=string_io, end="\n")

        print("// arcs: source -> dest [label]", file=string_io, end="\n")
        print(self._arcs_dot_representation(), file=string_io, end="")

        print("// start nodes", file=string_io, end="\n")
        print("\"{0}\" [style=filled];".format(self.initial_state), file=string_io, end="\n")

        print("// final nodes", file=string_io, end="\n")
        for state in self.final_states:
            print("\"{0}\" [peripheries=2];".format(state), file=string_io, end="\n")
    ...
    def __str__(self):
        string_io = StringIO()
        if self.name:
            print(self.name, file=string_io, end=" ")
        print("transducer:".format(), file=string_io, end="\n")
        print("initial state: {0}".format(self.initial_state), file=string_io, end="\n")
        print("final states: {0}".format([str(state) for state in self.final_states]), file=string_io, end="\n")
        print("states: {0}".format([str(state) for state in self.states]), file=string_io, end="\n")
        print("arcs:", file=string_io, end="\n")
        for arc in self._arcs:
            print(arc, file=string_io, end="\n")
        print("arcs_by_state_dict:", file=string_io, end="\n")
        for state1 in self.arcs_by_state_dict:
            for state2 in self.arcs_by_state_dict[state1]:
                print(str(state1)+" "+str(state2), file=string_io, end=": ")
